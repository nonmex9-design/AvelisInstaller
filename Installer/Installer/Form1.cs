...
namespace Installer
{
    public partial class Form1 : Form
    {
        ...
        private const string InstallerVersionUrl = "https://aveliss.pages.dev/download/installer-version.txt";
        ...
        private void HookDrag(Control parent)
        {
            ...
            {
                if (c == guna2Button1 || c == guna2ControlBox_close)
                    continue;
        ...
        private async void SetInstallerVersionLabel()
        {
            try
            {
                string installerVersion = await Task.Run(() =>
                {
                    using (var wc = new WebClient())
                    {
                        wc.Headers.Add("User-Agent", "Avelis-Bootstrapper/1.0");
                        return wc.DownloadString(InstallerVersionUrl).Trim();
                    }
                });

                if (InvokeRequired)
                {
                    Invoke(new Action(() => label17.Text = $"Installer {installerVersion}"));
                }
                else
                {
                    label17.Text = $"Installer {installerVersion}";
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Installer version fetch error: {ex.Message}");
                if (InvokeRequired)
                    Invoke(new Action(() => label17.Text = "Installer Unknown"));
                else
